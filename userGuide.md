...
在运行管理器时，若添加参数-local_console <控制台的路径>，就可以进入console调试模式**（注意：后面一定要加控制台路径）**

该模式就类似于命令行，可用于在面板服中更专业地配置服务器

### 7.客户端模式

可以把MCGA作为自启动服务器，这样就没法通过常规方式访问到本地控制台，这时候可以用客户端模式启动MCGA，这样MCGA会自动连接当前启动的远程控制台

### 8.远程控制台

启用远程控制台之后，可以通过telnet直接连接相应端口执行指令

如果是本地连接则不需要身份验证，否则需要输入账号密码
